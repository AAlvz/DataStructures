...
/***     Programa: ADT_OBJ_DIN_LISTA_H
         Programa Base: Ninguno
         Programador: Alfonso Alvarez
         Fecha: 16 - Feb - 09 2:00p.m.
...
template <class T> void Lista<T>::setElemOrd(T e) {
        Nodo<T> *tmp, *aux;
        if(!full()){
             tmp = new Nodo<T>(e);
             if(!empty()){
...
/***     Elemento: buscar
    ...
    Pre-Condicion: Lista inicializado  
   Post-Condicion: ninguna                                           ***/
template<class T> Nodo<T> * Lista<T>::buscar(T e){
...
template <class T> Nodo<T> * Lista<T> :: getPri(){
         Nodo <T> *apt;
         apt = lst;
         return apt;         
}
